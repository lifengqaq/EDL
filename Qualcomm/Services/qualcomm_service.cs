...
// Copyright (c) 2025-2026 lfEDL | Licensed under CC BY-NC-SA 4.0
// ============================================================================
...
using lfEDL.Qualcomm.Authentication;
// 已合并到 lfEDL.Qualcomm.Common 和 lfEDL.Qualcomm.Protocol

namespace lfEDL.Qualcomm.Services
{
    /// <summary>
    /// 连接状态
    /// </summary>
    ...
    public class QualcommService : IDisposable
    {
        ...
        /// <summary>
        /// 小米签名提供者 (用于交互式认证，返回签名 Base64)
        /// </summary>
        public Func<string, Task<string>> XiaomiSignatureProvider { get; set; }
            ...
            string authMode = "none", CancellationToken ct = default(CancellationToken))
        {
            try
            {
                ...
                if (authModeLower == "xiaomi" || (authModeLower == "none" && IsXiaomiDevice()))
                {
                    ...
                    xiaomi.SignatureProvider = XiaomiSignatureProvider;
        ...
        /// 使用签名进行小米认证 (在线授权流程)
        /// </summary>
        public async Task<bool> AuthenticateXiaomiSignatureAsync(string signatureBase64, CancellationToken ct = default(CancellationToken))
        {
            if (_firehose == null)
            {
                _log("[高通] 未连接到 Firehose 模式，无法执行认证");
                return false;
            }
            ...
            {
                var xiaomi = new XiaomiAuthStrategy(_log);
                bool success = await xiaomi.AuthenticateWithSignatureAsync(_firehose, signatureBase64, ct);

                if (success)
                {
                    _log("[高通] 小米在线认证成功");
                    // 认证成功后可能需要重新配置 Firehose? 或者是直接可用?
                    // 通常认证成功后设备解除锁定，直接可用。
                }
                else
                {
